...
fn run_all(verbose: bool) {
    ...
    // Write results to docs/COMPATIBILITY.md, preserving hand-written header.
    let compat_path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("docs/COMPATIBILITY.md");
    let marker = "<!-- AUTO-GENERATED BELOW — do not edit below this line -->";

    let existing = std::fs::read_to_string(&compat_path).unwrap_or_default();
    let header = if let Some(pos) = existing.find(marker) {
        // Keep everything up to and including the marker
        &existing[..pos + marker.len()]
    } else {
        // No marker found — use the whole file as header
        existing.trim_end()
    };
    ...
    std::fs::write(&compat_path, &file_content).expect("failed to write docs/COMPATIBILITY.md");
